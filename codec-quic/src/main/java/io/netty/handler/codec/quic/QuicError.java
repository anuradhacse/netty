...
/**
 * All error codes identified by the QUIC spec. QuicErrorCodes is encoded as a single octet on-the-wire.
 */
public enum QuicError {
    ...
    private static final QuicError[] INT_TO_ENUM_MAP;

    static {
        QuicError[] errors = QuicError.values();
        QuicError[] map = new QuicError[errors.length];
        for (QuicError error : errors) {
            map[(int) error.errorCode] = error;
        }
        INT_TO_ENUM_MAP = map;
    }

    QuicError(long errorCode) {
        this.errorCode = errorCode;
    }

    public static QuicError valueOf(long value) {
        return value >= INT_TO_ENUM_MAP.length || value < 0? null : INT_TO_ENUM_MAP[(int) value];
    }

    //todo error codes should convert to hexa decimal form
